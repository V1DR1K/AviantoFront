...
import type { CategoriaCatalogoResponse, ClienteResponse, FichaItemRequest, FichaResponse, ItemCatalogoResponse, MarcaMotoResponse, MotovehiculoResponse, PageResponse } from "../lib/types";
...
export function FichaForm({ onClose, onSave }: { onClose: () => void; onSave: (ficha: FichaResponse) => void }) {
  ...
  useEffect(() => { if (!clientId) return; void api<PageResponse<MotovehiculoResponse>>("/motovehiculos", {}, { clienteId: clientId, activo: true, size: 100 }).then((page) => { setVehicles(page.content); setVehicleId(page.content[0]?.id ?? ""); }).catch((reason) => setError(reason instanceof Error ? reason.message : "No se pudieron cargar las motos.")); }, [clientId]);
  ...
  const save = async () => {
    ...
    try {
      ...
      const ficha = await api<FichaResponse>("/fichas", { method: "POST", body: JSON.stringify(request) });
      await Promise.all(photos.map(async ({ file }) => api(`/api/fichas/${ficha.id}/fotos`, { method: "POST", body: JSON.stringify({ filename: file.name, contentType: "image/webp", base64: await readAsBase64(file) }) })));
      onSave(photos.length ? await api<FichaResponse>(`/api/fichas/${ficha.id}`) : ficha);
  ...
  return <section className="order-form">
    <div className="page-heading"><div><h1>Nueva ficha de trabajo</h1><p>Registrá el diagnóstico, los ítems y las condiciones del trabajo.</p></div><button className="button secondary form-close" onClick={() => hasDraftContent ? setCloseConfirmation(true) : onClose()}><X size={18} />Cerrar</button></div>
    ...
    <div className="form-layout"><div className="form-stack">
      ...
      <section className="form-section"><h2>2. Ítems y reparación</h2><label className="catalog-search">Buscar en catálogo<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Ej.: cambio de aceite" />{catalog.length > 0 && <div className="suggestions">{catalog.map((item) => <button key={item.id} type="button" onClick={() => addItem(item)}><span>{item.descripcion}</span><small>{item.tipo} · {money(item.precioBase)}</small></button>)}</div>}</label><div className="line-items">{items.map((item) => <div className="line-item" key={item.key}><div><strong>{item.descripcion}</strong><span>{item.tipo}</span></div><label>Cant.<input type="number" min="1" step="1" value={Number(item.cantidad)} onChange={(event) => updateLine(item.key, { cantidad: Number(event.target.value) })} /></label><label>Precio<input type="text" inputMode="decimal" value={priceInput(item.precioUnitario)} onChange={(event) => updateLine(item.key, { precioUnitario: parsePrice(event.target.value) })} /></label><strong>{money(Number(item.cantidad) * Number(item.precioUnitario) - Number(item.descuento))}</strong><button type="button" aria-label={`Eliminar ${item.descripcion}`} onClick={() => setItems((all) => all.filter((line) => line.key !== item.key))}><X size={18} /></button></div>)}</div><button className="text-button" type="button" onClick={() => setNewCatalogOpen(true)}><Plus size={17} />Agregar ítem personalizado</button></section>
