...
export function FichaForm({ onClose, onSave, fichaKey }: { onClose: () => void; onSave: (ficha: FichaResponse) => void; fichaKey?: string | null }) {
  ...
  const searchByPlate = async () => {
    ...
    if (!plate) return;
    setPatenteBusy(true); setError(null);
    try {
      const page = await api<PageResponse<MotovehiculoResponse>>("/motovehiculos", {}, { q: plate, activo: true, size: 20 });
  ...
  const save = async () => {
    if (!clientId || !vehicleId) return setError("Seleccioná un cliente y una moto.");
    if (!trabajos.length) return setError("Cargá al menos un trabajo a realizar.");
    const validWork = trabajos.filter((trabajo) => trabajo.descripcion.trim());
    if (!validWork.length) return setError("Cada trabajo necesita una descripción.");
    if (!editable) return setError(`La ficha ya está en "${loadedEstado}" y no puede editarse desde el formulario.`);
    ...
    try {
      const request: FichaRequest = {
        ...
        kilometrajeIngreso: kilometrajeIngreso ? Number(kilometrajeIngreso) : undefined,
