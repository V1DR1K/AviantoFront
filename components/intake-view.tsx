  ...
  return <Dialog open={open} title="Ingresar una moto" onClose={onClose} wide className="intake-modal">
     <p>Buscá la moto y elegí el destino operativo antes de abrir su perfil.</p>
      <section className="panel intake-card">
        ...
        {searched && !profile && <section className="intake-new-profile"><h3>Perfil nuevo</h3><div className="two-col"><SelectField label="Marca" value={values.marcaId ?? ""} onChange={(value) => set("marcaId", value)} placeholder="Seleccionar" options={brandOptions} required /><label className="intake-field">Modelo<input value={values.modelo ?? ""} onChange={(event) => set("modelo", event.target.value)} placeholder="Ej.: Wave 110, FZ 25..." autoComplete="off" required /></label><label className="intake-field">Año<input type="number" min="1900" max="2100" value={values.anio ?? ""} onChange={(event) => set("anio", event.target.value)} /></label><label className="intake-field">Kilometraje actual<input type="number" min="0" value={values.kilometraje ?? ""} onChange={(event) => set("kilometraje", event.target.value)} /></label><SelectField label="Cliente" value={values.clienteId ?? ""} onChange={(value) => set("clienteId", value)} placeholder="Seleccionar" options={clientOptions} required /><button type="button" className="button secondary" onClick={() => setNewClientOpen(true)}><Plus size={17} />Agregar cliente</button><label className="intake-field form-field-wide">Observaciones<textarea value={values.observaciones ?? ""} onChange={(event) => set("observaciones", event.target.value)} /></label></div></section>}
     </section>
