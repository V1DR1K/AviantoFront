...
export function IntakeView({ open, onClose, onOpenProfile, notify }: { open: boolean; onClose: () => void; onOpenProfile: (id: string) => void; notify: Notify }) {
  ...
  return <Dialog open={open} title="Ingresar una moto" onClose={onClose} wide className="intake-modal">
     ...
     <section className="panel intake-card">
       ...
       {searched && !profile && <section className="intake-new-profile"><h3>Perfil nuevo</h3><div className="two-col"><SelectField label="Marca" value={values.marcaId ?? ""} onChange={(value) => set("marcaId", value)} placeholder="Seleccionar" options={brands.map((brand) => ({ value: brand.id, label: brand.nombre }))} required /><label>Modelo<input value={values.modelo ?? ""} onChange={(event) => set("modelo", event.target.value)} /></label><SelectField label="Cliente" value={values.clienteId ?? ""} onChange={(value) => set("clienteId", value)} placeholder="Seleccionar" options={clients.map((client) => ({ value: client.id, label: client.nombre }))} required /><button type="button" className="button secondary" onClick={() => setNewClientOpen(true)}><Plus size={17} />Agregar cliente</button></div></section>}
     </section>
