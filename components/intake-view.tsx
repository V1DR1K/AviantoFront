...
import { integerInput, parseIntegerInput, yearInput } from "../lib/format";
...
export function IntakeView({ open, initialPlate, onClose, onOpenProfile, canStartSale = false, notify }: { open: boolean; initialPlate?: string; onClose: () => void; onOpenProfile: (id: string) => void; canStartSale?: boolean; notify: Notify }) {
  ...
  const submit = async () => {
    ...
    if (section === "VENTA" && !canStartSale) return notify("Solo Administración puede ingresar una moto a Ventas.", "warning");
    setBusy(true);
    try {
