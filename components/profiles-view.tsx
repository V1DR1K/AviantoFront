...
export function ProfilesView({ onNew, onOpen, notify }: { onNew: () => void; onOpen: (id: string) => void; notify: Notify }) {
  ...
  const deliver = async (profile: PerfilResponse) => {
    try { await api(`/motovehiculos/${profile.id}/entrega`, { method: "POST" }); setReloadKey((key) => key + 1); notify("Moto entregada."); }
    catch (reason) { const message = reason instanceof Error ? reason.message : "No se pudo entregar la moto."; setError(message); notify(message, "error"); }
  };
