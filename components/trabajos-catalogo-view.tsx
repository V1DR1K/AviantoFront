...
import { ConfirmModal, Dialog, EmptyState, FilterBar, SearchBox, SelectField, type Notify } from "./ui";
...
export function TrabajosCatalogoView({ notify }: { notify: Notify }) {
  ...
  return <AviantoPage className="work-catalog-page">
    <AviantoPageHeader title="Trabajos" description="Precios de referencia para presupuestar las fichas del taller." eyebrow="Catálogo" actions={<button className="button primary" onClick={() => setEditing({ id: "", descripcion: "", precioBase: 0, activo: true })}><Plus size={19} />Nuevo trabajo</button>} />
    <AviantoPanel className="table-panel work-catalog-table-panel">
      <div className="panel-head"><div><h2>Precios de referencia</h2><p>Valores sugeridos para presupuestos de taller.</p></div></div>
       <FilterBar primary={<SearchBox value={query} onChange={setQuery} placeholder="Buscar por descripción" />} activeCount={filter !== "Todos" ? 1 : 0}>
