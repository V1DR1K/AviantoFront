...
export function TrabajosCatalogoView({ notify }: { notify: (message: string) => void }) {
  ...
  const load = () => {
    ...
    }).then(setRows).catch((reason) => setError(errorMessage(reason)));
  };
  ...
  return <div className="page">
    ...
    <AbmFormModal key={editing?.id || "new"} open={editing !== null} resource="trabajo" mode={editing?.id ? "modificar" : "agregar"} initialValues={editing ?? {}} fields={[{ key: "descripcion", label: "Descripción", required: true, wide: true }, { key: "precioBase", label: "Precio sugerido", type: "currency", required: true }]} onClose={() => setEditing(null)} onSubmit={submit} />
    ...
    <ConfirmModal open={deleting !== null} title="Eliminar trabajo" body={`Vas a dar de baja "${deleting?.descripcion ?? ""}". No se sugerirá en nuevas fichas, pero los trabajos históricos se conservarán.`} confirmLabel="Eliminar trabajo" onClose={() => setDeleting(null)} onConfirm={() => { const selected = deleting; if (!selected) return; setDeleting(null); void api(`/configuracion/trabajos/${selected.id}`, { method: "DELETE" }).then(() => { load(); notify("Trabajo eliminado."); }).catch((reason) => setError(errorMessage(reason))); }} />
  </div>;
}
