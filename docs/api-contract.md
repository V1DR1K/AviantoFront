...
| Fichas | `GET/POST /fichas`, `GET/PUT/DELETE /fichas/{id}`, `GET /fichas/{id}/repuestos`, `GET /fichas/{id}/trabajos`, `POST/PUT/DELETE /fichas/{id}/trabajos`, `PATCH /fichas/{id}/estado`, `GET/POST /fichas/{id}/pagos`, `POST /fichas/{id}/pagos/{pagoId}/anular`, `GET /fichas/{id}/revision`, `POST /fichas/{id}/revision/preparar`, `PATCH /fichas/{id}/revision/controles/{controlId}`, `POST /fichas/{id}/revision/aprobar`, `POST /fichas/{id}/entregar` |
| Ventas | `GET /ventas`, `GET /ventas/{id}`, `GET /motovehiculos/{id}/venta`, `POST /ventas/{id}/checklist/sincronizar`, `PATCH /ventas/{id}/items/{itemId}`, `PUT /ventas/{id}/comprador`, `POST /ventas/{id}/transferencia`, `POST /ventas/{id}/transferencia/cancelar`, `PUT /ventas/{id}/transferencia/cita`, `POST /ventas/{id}/transferencia/asistencia`, `POST /ventas/{id}/completar` |
...
- `VentaChecklistPlantillaRequest`: `{ etiqueta, orden, obligatorio, activo }`. `VentaChecklistPlantillaResponse` agrega `id`, `createdAt`, `updatedAt`. Las plantillas activas se incorporan durante el ingreso a Ventas o mediante `POST /ventas/{id}/checklist/sincronizar`; los estados existentes de la carpeta se conservan.
...
- Iniciar transferencia exige comprador prospectivo y checklist obligatorio realizado. Una transferencia no finalizada puede cancelarse desde `POST /ventas/{id}/transferencia/cancelar`: la moto vuelve a `En venta`, el comprador prospectivo se libera y el registro conserva su auditoría. Completar la venta exige además cita completa, no futura para registrar asistencia, asistencia confirmada y propietario actual coincidente con el vendedor. Una venta finalizada no impide que la moto vuelva a ingresar a Taller o que inicie una nueva operación de Venta.
- Los endpoints `GET` de ventas y revisión son de lectura: no crean ni actualizan snapshots. La inicialización o sincronización se solicita explícitamente con `POST /ventas/{id}/checklist/sincronizar` y `POST /fichas/{id}/revision/preparar`.
