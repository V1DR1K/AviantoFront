...
| Fichas | `GET/POST /fichas`, `GET/PUT/DELETE /fichas/{id}`, `GET /fichas/{id}/repuestos`, `PATCH /fichas/{id}/estado`, `PATCH /fichas/{id}/pago`, `POST /fichas/{id}/entregar` |
...
- `PATCH /fichas/{id}/pago` recibe `{ estadoPago: "No pagado" | "Parcial" | "Pagado", itemIds?: UUID[] }`. Todos los trabajos no cancelados son cobrables, sin importar si están pendientes o realizados. `Pagado` marca todos los trabajos cobrables, `Parcial` requiere uno o más `itemIds` de esos trabajos y `No pagado` los desmarca. Una ficha cancelada no admite pagos. Los repuestos se cobran por separado con `PATCH /repuestos/{id}/pago`.
