...
| Motovehículos | `GET/POST /motovehiculos`, `GET/PUT/DELETE /motovehiculos/{id}`, `GET /motovehiculos/autocomplete?q=`, `POST /motovehiculos/{id}/ingreso`, `POST /motovehiculos/{id}/entrega`, `POST /motovehiculos/{id}/venta/completar` |
...
- El perfil expone la sección actual (`Taller` o `Venta`), la presencia física (`ingresada`) y el estado operativo. Taller usa `Ingresada Taller`, `Cargada`, `En proceso`, `En revisión`, `Entregada`; ingresar directamente en Ventas crea la moto en `En venta`, seguida por `Transferencia en curso` y `Vendida`.
