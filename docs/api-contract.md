...
| Motovehículos | `GET/POST /motovehiculos`, `GET/PUT/DELETE /motovehiculos/{id}`, `GET /motovehiculos/autocomplete?q=`, `POST /motovehiculos/{id}/ingreso`, `POST /motovehiculos/{id}/entrega`, `PATCH /motovehiculos/{id}/venta/estado`, `POST /motovehiculos/{id}/venta/completar` |
...
- El perfil expone la sección actual (`Taller` o `Venta`), la presencia física (`ingresada`) y el estado operativo. Taller usa `Ingresada Taller`, `Cargada`, `En proceso`, `En revisión`, `Entregada`; Ventas usa `Ingresada Venta`, `En venta`, `Transferencia en curso`, `Vendida`.
